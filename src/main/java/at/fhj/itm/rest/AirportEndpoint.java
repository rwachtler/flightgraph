...
public class AirportEndpoint
{
   ...
   public Response create(Airport entity)
   {
      em.persist(entity);
   ...
   @GET
   @Produces("application/json")
   public List<Airport> listAll(@QueryParam("start") Integer startPosition, @QueryParam("max") Integer maxResult)
   {
      ...
      final List<Airport> results = findAllQuery.getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(@PathParam("id") Long id, Airport entity)
   {
      if (entity == null)
      {
      ...
      if (em.find(Airport.class, id) == null)
      {
         return Response.status(Status.NOT_FOUND).build();
      }
      try
